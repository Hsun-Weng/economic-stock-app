...
const StockIndexCandleStickChart = ({ indexCode }) => {
    const [ stockIndexPrices, setStockIndexPrices ] = useState([]);
    ...
    useEffect(() => {
        const fetchData = () => {
            ...
            fetch(`/api/stock/index/${indexCode}/prices?startDate=${formatDate(startDate)}&endDate=${formatDate(endDate)}`)
                .then((res)=>res.json())
                .then((res)=>res.data)
                .then((data)=>setStockIndexPrices(data))
        }
        fetchData();
    }, [ indexCode ]);

    return (
        <ReactEcharts option={getOption(stockIndexPrices)} />
    )
}

export default StockIndexCandleStickChart;